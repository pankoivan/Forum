...
public class AuthorityServiceImpl implements AuthorityService {
    ...
    public Authority findById(Integer id) throws ServiceException {
        return repository.findById(id)
                .orElseThrow(() -> new ServiceException("Authority with id \"%s\" doesn't exists".formatted(id)));
    }
    ...
    public String deletingValidation(Authority authority) {
        return authority.getRoles().stream()
                ...
                ? "Право \"%s\" нельзя удалять, так как для каких-то ролей оно является единственным".formatted(authority.getName())
                : null;
    }
